...
public class TAddTournois extends BaseForm {
    ...
    public TAddTournois(com.codename1.ui.util.Resources resourceObjectInstance) {
    ...
    private void initGuiBuilderComponents(Resources resourceObjectInstance) {
        ...
        setTitle("Ajouter Tournois");
        setName("AjoutTournois");
        ...
        btnconfirmer.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                if (!nomText.getText().isEmpty() && !datePickerdebut.getText().isEmpty() && !datePickerfin.getText().isEmpty() && !primeText.getText().isEmpty()) {
                    ...
                    tournois.setNom(nomText.getText());

                    tournois.setDate_debut(datePickerdebut.getDate());
                    tournois.setDate_fin(datePickerfin.getDate());
                    tournois.setPrime(Integer.parseInt(primeText.getText().toString()));

                    ServiceTournois su = ServiceTournois.getInstance();

                    if (su.add(tournois)) {
                        Dialog.show("Tournois", "Tournois ajouté", new Command("OK"));

                    } else {
                        Dialog.show("Tournois", "Erreur d'ajout", new Command("OK"));
                    }
                    new TTournois().show();
